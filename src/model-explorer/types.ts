...
export interface ModelExplorerConfig {
  ...
  // Camera far clip plane. Default 5000 (past the sky sphere's own
  // default radius of 2500) was fine for Boeing's ~90-unit scale, but a
  // model at a much larger scale can have its own geometry sit farther
  // from the camera than 5000 units at a normal viewing distance — parts
  // of it get silently clipped out of the frame entirely, not just
  // fogged. Checked by hand for the Merlin config: default
  // camera-to-target distance (~3900) plus the model's own radius
  // (~1700) exceeds 5000.
  cameraFar?: number;
  // Radius of the GradientSky sphere (see GradientSky.tsx). Must stay
  // safely under cameraFar and safely over controlsLimits.maxDistance —
  // the camera needs to always be inside it. Default 2500 suits Boeing's
